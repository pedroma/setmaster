...
# There are some cards where the number is "None" because they don't really have a number (old cards)

for card in cards:
    query = {"gatherer": re.compile(r"^{0}$".format(card["front"]["set"]), re.IGNORECASE)}
    expansion = mongo.sets.find_one(query)
    number = card["front"]["number"]
    logger.debug("Processing [{0}] from [{1}] {2}".format(card["front"]["name"].encode('utf8'), card["front"]["set"], number))
    url = "http://magiccards.info/scans/en/{0}/{1}.jpg".format(expansion["mtgcardsinfo"], number)
    if number != "None":
        ...
        if not os.path.exists("{0}/{1}.png".format(expansion_dir, card["front"]["multiverse_id"])):
            ...
            try:
                ...
                logger.error("{0} [{1}] [{2}]".format(url, card["front"]["name"].encode('utf8'), number))
    else:
        ...
        logger.warn("{0} [{1}] [{2}]".format(url, card["front"]["name"].encode('utf8'), number))
